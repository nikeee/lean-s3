...
To support those runtimes, the HTTP layer (`src/http.ts`) hands out per-client `HttpClient` handles (`createHttpClient()`, one per `S3Client`) that abstract over a small `request()` primitive and select a backend at runtime:

- **Node.js:** dynamically imports `undici` and uses `undici.request` with an `Agent` owned by that client - the original, fast path. Owning the `Agent` per client is what makes `S3Client.close()` / `await using` (`Symbol.asyncDispose`) meaningful: closing one client's connection pool doesn't affect other clients. `undici` is declared as an `optionalDependency`, so installs that skip optional deps (or run on a runtime where it can't be built) still work.
- **Bun (and any runtime without a usable undici):** falls back to the global `fetch`, normalized to the same response shape that the rest of the codebase expects (`statusCode`, a plain lower-cased `headers` object, and a `body` exposing `text()`/`dump()` plus a `Readable`-style `on("data" | "end" | "error")` and `pause()`/`resume()` for streaming with backpressure). The streaming path reads the web `ReadableStream` directly (no `Readable.fromWeb` round-trip) to keep reads cheap. `close()` is a no-op here, since `fetch`'s connection pool is managed by the runtime.

The backend is resolved once per client and cached, so the per-request hot path stays cheap. This keeps Node.js on the high-performance undici path while making lean-s3 usable on Bun without code changes by the consumer.

The `GetObject` streaming path applies backpressure: when the consumer of the returned `ReadableStream` falls behind, the source body is `pause()`d and later `resume()`d (see `src/backpressure.test.ts`). On the undici backend this propagates all the way to the socket. On Bun, the JS-side queue is still bounded, but Bun's native `fetch` currently buffers the response body eagerly and does not propagate backpressure to the socket - that part is outside our control.

Benchmarks against a local MinIO show Bun (with its natively-implemented `fetch`) is at parity with - and on some operations like `HeadObject` notably faster than - Node.js with undici. (Note: Node.js's `fetch` is itself implemented on top of undici with extra browser-oriented overhead, so on Node.js the direct undici path is meaningfully faster than `fetch` - that's why undici remains the default there. One caveat: undici `8.10.0` re-introduced a ~1 ms idle-socket-validation delay per sequential keep-alive request; see the note in [BENCHMARKS.md](./BENCHMARKS.md#nodejs-undici-vs-bun-fetch).)
